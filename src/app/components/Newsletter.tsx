  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEmail(''); // সফল সাবমিট শেষে ইনপুট ফিল্ড খালি করা
      } else {
        ...
        <div className="flex-1 w-full max-w-xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 w-full">
            <input
              ...
              placeholder="Enter your email"
              required
              className="flex-1 px-5 py-3 rounded-xl border border-slate-200 bg-white text-sm text-slate-800 focus:outline-none focus:border-blue-500 disabled:opacity-60 transition-colors shadow-sm"
            />
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-medium text-sm rounded-xl transition-colors shadow-sm flex items-center justify-center gap-2 whitespace-nowrap min-w-[120px]"
            >
              {isLoading ? (
                <>
